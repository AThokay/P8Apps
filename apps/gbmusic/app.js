...
function startButtonWatches() {
  TC.on("touch",(p)=> {
    if (p.x>180 && p.y>0 && p.x<240 && p.y<60){
    sendCommand("volumeup");
    }
    else if (p.x>180 && p.y>180 && p.x<240 && p.y<240) sendCommand("volumedown");
    else if (p.x>60 && p.y>180 && p.x<180 && p.y<240) togglePlay();
  });
}
...
function startTouchWatches() {
  TC.on("swipe",(dir)=>{
  if (dir==TC.RIGHT) sendCommand("previous");
  else if (dir==TC.LEFT) sendCommand("next");
  });
}
