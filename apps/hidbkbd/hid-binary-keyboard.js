...
var s = storage.readJSON("settings.json",1)||{ HID: false };
const KEY = {
  A           : 4 ,
  B           : 5 ,
  C           : 6 ,
...
function show(chars,callback) {
  ...
  showChars(120,charr);
    TC.on("swipe",(dir)=>{
  if (dir==TC.RIGHT) show(charl,callback); // space;
  else if (dir==TC.LEFT)  show(charr,callback); // enter;
  });
}
...
if (s.HID=="kb" || s.HID=="kbmedia") {
  ...
  setTimeout(startKeyboardHID,500);
  TC.on("touch",(p)=> {
    if (p.x>0 && p.y>0 && p.x<120 && p.y<240){
    sendHID(44);
    }
    else if (p.x>120 && p.y>0 && p.x<240 && p.y<240) sendHID(40);
  });
} else {
